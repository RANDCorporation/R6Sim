...
The `R6Sim` class is meant to represent an R6 simulation model. An `R6Sim` model should be self-contained model, and once instantiated, should immediately know how to simulate itself with the `R6Sim$simulate()` function. Here is an example `R6Sim` model:
...
```
Once you experiment runs, now you can iterate and add new inputs, or new parameters to your experiment. To see a real use case with parallel runs, see the [value-of-surveillance](github.com/randcorporation/value-of-surveillance) project.
